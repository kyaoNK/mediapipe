...
constexpr char kOutputMultiLandmarks[] = "multi_face_landmarks";
...
::mediapipe::Status RunMPPGraph() {
  ...
  ASSIGN_OR_RETURN(mediapipe::OutputStreamPoller poller_landmarks,
                   graph.AddOutputStreamPoller(kOutputMultiLandmarks));
  ...
  while (grab_frames) {
    ...
    // time
    /*
    std::clock_t clock_now = clock();
    std::cout << "time: " << (double)(clock_now - clock_begin) / CLOCKS_PER_SEC << "sec ";
    */
    // check fps
    frame_cnt++;
    ...
    if(time_now - tick >= 1){
      tick++;
      std::cout << "FPS: " << frame_cnt << "  movie_sec: " << tick << "sec" << std::endl;
      frame_cnt = 0;
    }
